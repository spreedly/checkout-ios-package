...
// Create custom theme
struct CustomTheme: SpreedlyTheme {
    var colors: SpreedlyColors {
        return SpreedlyColors(
            ...
            placeholder: Color.gray.opacity(0.7)
        )
    }

    var typography: SpreedlyTypography {
        return SpreedlyTypography(
            titleFont: Font.system(size: 24, weight: .bold),
            subtitleFont: Font.system(size: 18, weight: .semibold),
            bodyFont: Font.system(size: 16, weight: .regular),
            captionFont: Font.system(size: 14, weight: .medium),
            buttonFont: Font.system(size: 18, weight: .semibold),
            fieldFont: Font.system(size: 16, weight: .regular)
        )
    }

    var spacing: SpreedlySpacing {
        return SpreedlySpacing(
            xs: 4, sm: 8, md: 16, lg: 24, xl: 32, xxl: 48
        )
    }

    var borderRadius: SpreedlyBorderRadius {
        return SpreedlyBorderRadius(
            xs: 4, sm: 8, md: 12, lg: 16, xl: 24
        )
    }

    var shadows: SpreedlyShadows {
...
The SpreedlyUI SDK includes a powerful global theme system that allows you to set themes once and have them apply to all components automatically.
...
// Create and set custom theme globally
let customTheme = SpreedlyThemeManager.createCustomTheme(
    colors: SpreedlyColors(primary: Color.red),
...
```objc
// Set global themes
[SpreedlyThemeManager setDarkThemeForObjectiveC];
[SpreedlyThemeManager setLightThemeForObjectiveC];
[SpreedlyThemeManager resetToDefaultThemeForObjectiveC];

// Create and set custom theme
SpreedlyColors *customColors = [[SpreedlyColors alloc] initWithPrimary:[UIColor redColor]];
id<SpreedlyTheme> customTheme = [SpreedlyThemeManager createCustomThemeWithColors:customColors];
[SpreedlyThemeManager setGlobalTheme:customTheme];
```
...
struct MyApp: App {
    init() {
        // Set global theme for entire app
        SpreedlyThemeManager.setDarkTheme()
    }
...
You can also set custom themes on individual components:
...
let textFieldVC = SPLTextFieldViewController(
    field: .cardNumber,
    title: "Card Number",
    isRequired: true,
...
// Or set theme after creation
textFieldVC.setTheme(customTheme)
textFieldVC.resetToGlobalTheme() // Reset to global theme
```
...
The theme system uses a clear priority hierarchy:

1. **Custom Theme** (Highest Priority) - Set on individual components
2. **Global Theme** (Medium Priority) - Set via `SpreedlyThemeManager`
3. **Default Theme** (Lowest Priority) - Fallback theme
...
1. **Screenshot Prevention** - Prevents screenshots of sensitive views using iOS secure text field protection
2. **Screen Recording Prevention** - Detects and protects content when screen recording or mirroring is active
3. **App Switcher Prevention** - Shows a privacy overlay when the app appears in the app switcher preview

#### SwiftUI Integration

**Note:** `CardFormDropIn` already includes built-in screen prevention protection, so you don't need to apply `.screenPrevention()` to it. Use the modifier on other views that need protection.

For other SwiftUI views, use the `.screenPrevention()` modifier:
...
struct PaymentView: View {
    var body: some View {
        VStack {
            // CardFormDropIn already has screen prevention built-in
            CardFormDropIn(
                onProcessingResult: { result in
            ...
            // Apply protection to other sensitive views
            SensitiveDataView()
                .screenPrevention()
        }
    }
...
You can customize the text shown in screenshots/recordings:
...
struct PaymentView: View {
    var body: some View {
        VStack {
            ...
            // Custom view with placeholder text
            CustomSensitiveView()
                .screenPrevention(placeholderText: "This content is protected")
        }
    }
}
```
...
Apply protection to your entire app in the main app file:
...
struct ContentView: View {
    ...
    var body: some View {
        VStack {
            ...
            // CardFormDropIn already has built-in protection
            
            Button("Show Sensitive Data") {
                showSensitiveData = true
            }
        }
        .sheet(isPresented: $showPaymentForm) {
            // CardFormDropIn has built-in screen prevention
            CardFormDropIn(
                onProcessingResult: { result in
                    if result.isSuccess {
        ...
        .sheet(isPresented: $showSensitiveData) {
            // Apply protection to custom views
            SensitiveDataSheet()
                .screenPrevention()
        }
    }
...
- (void)showPaymentForm {
    // CardFormDropInViewController already has built-in protection
    CardFormDropInViewController *paymentFormVC = [[CardFormDropInViewController alloc] init];
    
    // Present directly - no need to wrap
    [self presentViewController:paymentFormVC animated:YES completion:nil];
    
    // For custom view controllers, wrap them:
    // SensitiveDataViewController *sensitiveVC = [[SensitiveDataViewController alloc] init];
    // UIViewController *secureVC = [sensitiveVC wrapInSecureViewController];
...
The SDK uses iOS's secure text field mechanism (`isSecureTextEntry`) to prevent screenshots and screen recordings. When protection is enabled:

- Screenshots are blocked - attempting to take a screenshot will capture a blank screen or placeholder text
- Screen recording is detected - the SDK monitors `UIScreen.isCaptured` to detect active recording
- Content is protected - sensitive views are wrapped in a secure container that prevents visual capture

**App Switcher Protection:**

When your app goes to the background, the SDK automatically:

- Shows a privacy overlay (blur effect or custom text) in the app switcher preview
- Removes the overlay when the app becomes active again
- Handles multiple windows and scenes properly

**Automatic Lifecycle Management:**

The screen prevention manager automatically handles:

- App state changes (background/foreground transitions)
- Screen capture state changes (recording start/stop)
...
1. **CardFormDropIn is Already Protected**: The `CardFormDropIn` component includes built-in screen prevention, so you don't need to apply it manually
2. **Apply Protection to Custom Views**: Use `.screenPrevention()` on your own views that display sensitive payment information
3. **Use Placeholder Text**: Provide meaningful placeholder text for better user experience when screenshots are attempted
4. **Test on Device**: Test screenshot and recording prevention on real devices (not just simulator)
5. **App Switcher Testing**: Test app switcher preview by double-pressing home button or using gestures
6. **Multiple Windows**: Ensure protection works across all windows and scenes if your app supports them
