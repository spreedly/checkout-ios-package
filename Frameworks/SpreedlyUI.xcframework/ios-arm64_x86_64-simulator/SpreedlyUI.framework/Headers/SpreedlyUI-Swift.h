...
/// For card-number fields: <code>true</code> when some or all PAN digits are hidden in the field UI at emit time (Pretty + focused = <code>false</code>; Masked / Pretty on blur = <code>true</code>). Always <code>true</code> on non-PAN fields — ignore on CVC.
...
@property (nonatomic) NSInteger cardNumberFormatRawValue;
/// Per-field CVC display mask when <code>observeHostedCardDisplayState</code> is <code>NO</code> (default <code>YES</code> = bullets). Ignored for non-CVC fields.
@property (nonatomic) BOOL cvvDisplayMasked;
