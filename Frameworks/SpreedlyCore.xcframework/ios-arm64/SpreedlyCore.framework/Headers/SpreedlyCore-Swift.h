...
/// Clears secure values, validation errors, and in-flight payment subscriptions.
/// Does <em>not</em> reset hosted PAN/CVV display state (<code>setNumberFormat</code> / <code>toggleMask</code>). Use when dismissing
/// a payment sheet. Full form reset runs on sheet <em>open</em> via <code>resetPaymentState()</code>, not on dismiss.
- (void)resetPaymentSession;
/// Full payment form reset: secure values, validation, visible fields, and PAN/CVV display.
- (void)resetPaymentState;
/// Same as <code>resetPaymentState()</code> — kept for existing integrations.
- (void)reset;
/// Skip the next drop-in <code>resetPaymentState()</code> on appear (e.g. configuration change while sheet is visible).
/// <em>Call on the main thread only.</em> Pair with the drop-in’s <code>.onAppear</code> (which is main-thread).
- (void)preservePaymentStateOnNextShow;
/// Whether the next sheet open should preserve form state; consumes the one-shot preserve flag.
/// <em>Call on the main thread only.</em> Internal call site is <code>CardFormDropIn.onAppear</code>.
