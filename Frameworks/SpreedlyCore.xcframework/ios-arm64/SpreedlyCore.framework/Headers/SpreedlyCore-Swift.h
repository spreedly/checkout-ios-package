...
@class NSString;
SWIFT_ENUM_FWD_DECL(NSInteger, BraintreePaymentType)
...
SWIFT_ENUM_FWD_DECL(NSInteger, OffsitePaymentMethodType)
...
/// Merchants obtain a PayPal Client ID and merchant account ID from the PayPal developer dashboard.
...
/// PayPal merchant account identifier required by PayPal’s iOS SDK (<code>CoreConfig</code>).
...
/// Return URL registered for app-switch / Universal Link return after PayPal checkout.
...
- (nonnull instancetype)initWithClientId:(NSString * _Nonnull)clientId merchantId:(NSString * _Nonnull)merchantId environment:(enum PayPalEnvironment)environment returnURL:(NSURL * _Nonnull)returnURL merchantDisplayName:(NSString * _Nonnull)merchantDisplayName locale:(NSString * _Nullable)locale OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
/// Whether the funding source is eligible for the supplied currency, amount, and (Venmo) country.
...
/// Which PPCP funding sources to offer for a given cart.
/// Computed on-device from currency, country, and amount — PayPal’s iOS SDK has no eligibility
/// endpoint to ask. Use it to decide which buttons to draw; approval can still fail at checkout.
...
/// Vault browser / web-sheet presentation failed (ASWebAuthenticationSession / web session).
/// Produced by vault-flow mapping of PayPal <code>webSessionError</code> and presentation failures.
  PayPalErrorCodeVaultBrowserSwitchFailed = 5,
/// Vault session timed out before payment-token exchange completed.
/// Produced by vault-flow mapping of <code>URLError.timedOut</code>.
  PayPalErrorCodeVaultSessionExpired = 6,
/// Merchant provider returned an empty or missing setup token id.
  PayPalErrorCodeVaultSetupTokenMissing = 7,
/// Merchant provider failed to exchange setup token for a payment token.
  PayPalErrorCodeVaultPaymentTokenExchangeFailed = 8,
/// A previous PayPal session was interrupted (process death). Reconcile server-side before retrying.
  PayPalErrorCodeProcessDeathRecoveryPending = 9,
/// Process-death recovery storage could not be read or written.
  PayPalErrorCodeProcessDeathRecoveryStorageUnavailable = 10,
/// <code>initialize(config:)</code> was not called before starting a session.
  PayPalErrorCodeNotInitialized = 11,
/// PayPal checkout config failed validation.
  PayPalErrorCodeInvalidConfig = 12,
/// Selected funding source is not eligible for this cart.
  PayPalErrorCodeIneligibleFundingSource = 13,
/// Merchant <code>createOrder</code> failed or returned an empty order id.
  PayPalErrorCodeOrderCreationFailed = 14,
/// Merchant <code>captureOrder</code> failed.
  PayPalErrorCodeCaptureFailed = 15,
/// PayPal web-sheet / browser presentation failed.
  PayPalErrorCodeBrowserSwitchFailed = 16,
/// Return URL / callback validation failed.
  PayPalErrorCodeReturnValidationFailed = 17,
/// Venmo is not available for this session.
  PayPalErrorCodeVenmoNotSupported = 18,
/// PayPal <code>createPayPalSession()</code> was not called, or session creation failed, before <code>start</code>/<code>vault</code>.
  PayPalErrorCodeSessionNotStarted = 19,
/// PayPal app-switch return failed before buyer approval could complete.
  PayPalErrorCodeAppSwitchFailed = 20,
};
...
/// PayPal payment token id (<code>vault_id</code>) from a purchase-later vault session or recurring report.
/// Only present for native PayPal (PPCP) vault / recurring flows — never log the raw value.
@property (nonatomic, readonly, copy) NSString * _Nullable vaultId;
/// PayPal customer id associated with a vaulted payment token, when returned.
/// Only present for native PayPal (PPCP) vault / recurring flows.
@property (nonatomic, readonly, copy) NSString * _Nullable customerId;
/// Spreedly payment-method type for native PayPal flows:
/// <code>paypal_ppcp</code>, <code>paypal_vault</code>, or <code>paypal_vault_recurring</code>.
...
/// Same as <code>paypalCompleted</code> with an explicit <code>paymentMethodType</code>
/// (<code>paypal_ppcp</code>, <code>paypal_vault</code>, or <code>paypal_vault_recurring</code>).
+ (PaymentResult * _Nonnull)paypalCompletedWithTransactionToken:(NSString * _Nullable)transactionToken orderId:(NSString * _Nonnull)orderId payerId:(NSString * _Nullable)payerId fundingSource:(NSString * _Nonnull)fundingSource state:(NSString * _Nullable)state paymentMethodType:(NSString * _Nonnull)paymentMethodType SWIFT_WARN_UNUSED_RESULT;
/// Creates a successful payment result after purchase-later vault consent and payment-token exchange.
/// warning:
/// Deprecated. Vault PPCP outcomes use <code>paypalCompleted</code> with <code>vaultId</code> / <code>customerId</code> left nil.
+ (PaymentResult * _Nonnull)paypalVaultApprovedWithTransactionToken:(NSString * _Nullable)transactionToken vaultId:(NSString * _Nonnull)vaultId customerId:(NSString * _Nullable)customerId fundingSource:(NSString * _Nonnull)fundingSource state:(NSString * _Nullable)state SWIFT_WARN_UNUSED_RESULT SWIFT_DEPRECATED_MSG("Use paypalCompleted. Vault identity lives on the merchant backend.");
/// Creates a payment result for a merchant-reported native PayPal (PPCP) recurring charge outcome.
/// warning:
/// Deprecated. Recurring PPCP outcomes use <code>paypalCompleted</code> with <code>vaultId</code> left nil.
+ (PaymentResult * _Nonnull)paypalRecurringCompletedWithOrderId:(NSString * _Nullable)orderId vaultId:(NSString * _Nonnull)vaultId customerId:(NSString * _Nullable)customerId fundingSource:(NSString * _Nonnull)fundingSource state:(NSString * _Nullable)state SWIFT_WARN_UNUSED_RESULT SWIFT_DEPRECATED_MSG("Use paypalCompleted. Recurring identity lives on the merchant backend.");
...
SWIFT_ENUM_FWD_DECL(NSInteger, SecurityIntegrityLevel)
...
/// <code>nonce</code>, <code>signature</code>, <code>certificateToken</code>, and <code>timestamp</code> on the active config.
...
SWIFT_ENUM_FWD_DECL(NSInteger, SpreedlySecurityErrorCode)
...
+ (void)paypalVaultStartedWithUsageType:(NSString * _Nonnull)usageType rdaAvailable:(BOOL)rdaAvailable;
+ (void)paypalVaultApprovedWithDurationMs:(int64_t)durationMs;
+ (void)paypalVaultCancelledWithSource:(NSString * _Nonnull)source durationMs:(int64_t)durationMs;
+ (void)paypalVaultErrorWithErrorCode:(NSString * _Nonnull)errorCode durationMs:(int64_t)durationMs;
+ (void)paypalVaultOrphanPaymentTokenWithDurationMs:(int64_t)durationMs;
+ (void)paypalRecurringCompletedWithSuccess:(BOOL)success errorType:(NSString * _Nullable)errorType durationMs:(int64_t)durationMs;
...
+ (void)deviceDataCollectedWithProvider:(NSString * _Nonnull)provider success:(BOOL)success durationMs:(int64_t)durationMs errorType:(NSString * _Nullable)errorType;
